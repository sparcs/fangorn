...
public class QueryParserTest extends TestCase {
	public void testParsesApostrophies() throws Exception {
		QueryParser parser = new QueryParser(new String[]{"//", "n't"});
		final TreeExpr expr = parser.parse();
		assertNotNull(expr);
		final TreeTerm term = expr.getTerm(0);
		assertEquals(TreeAxis.DESCENDANT, term.axis());
		assertEquals("n't", term.termLabel());
	}
}
